...
function onOpen() {
  ...
  ui.createMenu('Checklist Tools')
    .addItem('Build All Bartender Days', 'buildAllBartenderDays')
    .addSeparator()
    .addItem('Build Bartender Day 1', 'buildBartenderDay1')
    .addItem('Build Bartender Day 2', 'buildBartenderDay2')
    .addItem('Build Bartender Day 3', 'buildBartenderDay3')
    .addItem('Build Bartender Day 4', 'buildBartenderDay4')
    .addItem('Build Bartender Day 5', 'buildBartenderDay5')
    .addSeparator()
...
function buildAllBartenderDays() {
  const ss = SpreadsheetApp.getActive();
  const days = [
    getBartenderDay1Data(),
    getBartenderDay2Data(),
    getBartenderDay3Data(),
    getBartenderDay4Data(),
    getBartenderDay5Data()
  ];
  days.forEach(cfg => buildDay_(ss, cfg));
  const first = ss.getSheetByName('Bartender Day 1');
  if (first) ss.setActiveSheet(first);
  ss.toast('All 5 Bartender checklists built.');
}

function buildBartenderDay1() { buildSingleDay_(getBartenderDay1Data()); }
function buildBartenderDay2() { buildSingleDay_(getBartenderDay2Data()); }
function buildBartenderDay3() { buildSingleDay_(getBartenderDay3Data()); }
function buildBartenderDay4() { buildSingleDay_(getBartenderDay4Data()); }
function buildBartenderDay5() { buildSingleDay_(getBartenderDay5Data()); }
...
function buildDay_(ss, cfg) {
  const sheetName = cfg.sheetName || `Bartender Day ${cfg.day}`;
...
function buildHeader_(sh, cfg) {
  ...
  // Rows 5-6: Meta fields on light gray band
  ...
  // Row 8: Thin navy accent bar (bottom rule)
  sh.getRange(row, 1, 1, 7).merge().setBackground(COLORS.navy);
