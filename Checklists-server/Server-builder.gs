// ============================================================
// CHECKLIST BUILDER ENGINE — SERVER
...
function onOpen() {
  ...
  ui.createMenu('Checklist Tools')
    .addItem('Build All Server Days', 'buildAllServerDays')
    .addSeparator()
    .addItem('Build Server Day 1', 'buildServerDay1')
    .addItem('Build Server Day 2', 'buildServerDay2')
    .addItem('Build Server Day 3', 'buildServerDay3')
    .addItem('Build Server Day 4', 'buildServerDay4')
    .addItem('Build Server Day 5', 'buildServerDay5')
    .addSeparator()
...
function buildAllServerDays() {
  ...
  const first = ss.getSheetByName('Server Day 1');
  if (first) ss.setActiveSheet(first);
  ss.toast('All 5 Server checklists built.');
}

function buildServerDay1() { buildSingleDay_(getServerDay1Data()); }
function buildServerDay2() { buildSingleDay_(getServerDay2Data()); }
function buildServerDay3() { buildSingleDay_(getServerDay3Data()); }
function buildServerDay4() { buildSingleDay_(getServerDay4Data()); }
function buildServerDay5() { buildSingleDay_(getServerDay5Data()); }
...
function buildDay_(ss, cfg) {
  const sheetName = cfg.sheetName || `Server Day ${cfg.day}`;
...
function writeSectionsSingleColumn_(sh, startRow, sections) {
  ...
  sections.forEach(section => {
    row = writeSectionBlock_(sh, row, section, 2, 3, 6);
    row += 1;
  });
  return row;
}
...
function writeSectionsTwoColumn_(sh, startRow, sections) {
  ...
  left.forEach(section => {
    leftRow = writeSectionBlock_(sh, leftRow, section, 2, 3, 3);
    leftRow += 1;
  });

  right.forEach(section => {
    rightRow = writeSectionBlock_(sh, rightRow, section, 5, 6, 6);
    rightRow += 1;
  });
